...
if (!window.marascott.anyBus_v3) {
	window.marascott.anyBus_v3 = {
		init: false,
		sync: false,
		input: {
			label: "0",
...
window.marascott.anyBus_v3.nodes = {}
window.marascott.anyBus_v3.flows = {
	start: [],
	list: [],
	end: [],
}
class MaraScottAnyBusNode_v3Widget {
...
class MaraScottAnyBusNode_v3 {
	...
	static connectBus(node, slot, node_origin, origin_slot) {
		...
		const isBusInput = slot == MaraScottAnyBusNode_v3.BUS_SLOT
		const isOutputs = node_origin.outputs?.length > 0
		let isMaraScottBusNode = node_origin.type == MaraScottAnyBusNode_v3.TYPE
		if (!isMaraScottBusNode) {
			const origin_reroute_node = MaraScottAnyBusNode_v3Flow.getOriginRerouteBusType(node_origin)
			isMaraScottBusNode = origin_reroute_node?.type == MaraScottAnyBusNode_v3.TYPE
			if (isMaraScottBusNode) {
		...
		const isOriginSlotBus = origin_slot == MaraScottAnyBusNode_v3.BUS_SLOT
		if (isBusInput && isOriginSlotBus && isOutputs && isMaraScottBusNode && (isOriginProfileSame || isTargetProfileDefault)) {
			...
			for (let _slot = MaraScottAnyBusNode_v3.FIRST_INDEX; _slot < node_origin.outputs.length; _slot++) {
	...
	static connectInput(node, slot, node_origin, origin_slot) {
		...
		const isOriginAnyBusBus = node_origin.type == MaraScottAnyBusNode_v3.TYPE
		const isOriginSlotBus = origin_slot == MaraScottAnyBusNode_v3.BUS_SLOT
...
class MaraScottAnyBusNode_v3Flow {
	...
	static syncProfile(node, isChangeWidget, isChangeConnect) {
		...
		if (!node.graph || window.marascott.anyBus_v3.sync == MaraScottAnyBusNode_v3Flow.NOSYNC) return
		if (window.marascott.anyBus_v3.sync == MaraScottAnyBusNode_v3Flow.FULLSYNC) {
	...
	static clean(node) {
		...
		if (node.inputs[MaraScottAnyBusNode_v3.BUS_SLOT].link != null) {
			...
			window.marascott.anyBus_v3.sync = MaraScottAnyBusNode_v3Flow.FULLSYNC
			MaraScottAnyBusNode_v3Flow.syncProfile(node, MaraScottAnyBusNode_v3Widget.CLEAN.name, false)
			MaraScottAnyBusNode_v3Flow.syncProfile(node, null, true)
...
class MaraScottAnyBusNode_v3LiteGraph {
...
const MaraScottAnyBusNode_v3Extension = {
	...
	name: "Comfy.MaraScott.AnyBusNode_v3",
	init(app) {
